...
public class FoldingNavigationLayout extends ViewGroup {
	...
	public FoldingNavigationLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public FoldingNavigationLayout(Context context, AttributeSet attrs, int defStyle) {
	...
	private void setFoldFactorNormal(float foldFactor) {
	...
	public void setFoldFactor(float foldFactor) {
		setFoldFactorNormal(1 - foldFactor);
	}
	...
	private void updateFold() {
		prepareFold(mAnchorFactor, mNumberOfFolds);
		calculateMatrices();
		invalidate();
	}
	 ...
	 * default.
	 */
	private void prepareFold(float anchorFactor, int numberOfFolds) {
		...
		int delta = Math.round(((float) w) / ((float) mNumberOfFolds));
	...
	private void calculateMatrices() {
		...
		float translatedDistance = mOriginalWidth * cTranslationFactor;
		 ...
		 * theorem.
		 */
		float depth = (float) Math.sqrt((double) (mFoldDrawWidth
				* mFoldDrawWidth - translatedDistanceFoldSquared));
		...
		float scaledWidth, scaledHeight, bottomScaledPoint, topScaledPoint;
		...
		float anchorPoint = mAnchorFactor * mOriginalWidth;

		/* The fold along which the anchor point is located. */
		float midFold = (anchorPoint / mFoldDrawWidth);
		...
		for (int x = 0; x < mNumberOfFolds; x++) {
			...
			mDst[0] = (anchorPoint > x * mFoldDrawWidth) ? anchorPoint
					+ (x - midFold) * scaledWidth : anchorPoint - (midFold - x)
					* scaledWidth;
			...
			mDst[4] = (anchorPoint > (x + 1) * mFoldDrawWidth) ? anchorPoint
					+ (x + 1 - midFold) * scaledWidth : anchorPoint
					- (midFold - x - 1) * scaledWidth;
